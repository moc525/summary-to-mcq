...
function App() {
  const { error, response, summarizeParaToMcq } = useOpenaiApi(); // Use the custom hook
  ...
  // useEffect to show alert when response changes
  useEffect(() => {
    if (response) {
      ...
      const timer = setTimeout(() => {
        setAlertVisible(false); // Hide the alert after 5 seconds
        setAlertError(false);
      }, 5000);
